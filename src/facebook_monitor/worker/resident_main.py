...
async def _run_resident_browser_runtime_session(
    *,
    ...
    facebook_runtime = FacebookAutomationRuntime()
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                executor = ExecutorWorkerPool(
                    ...
                    schedule_planner=schedule_planner,
                    scan_page=scan_page,
                    facebook_runtime=facebook_runtime,
                    **(
                        {
                            "comments_commit_ready_scan_page": comments_commit_ready_scan_page
                        }
                ...
                try:
                    ...
                    try:
                        ...
                        logger.exception("resident page-pool cleanup failed")
            finally:
...
async def run_resident_main_scheduler_tick(
    *,
    ...
    stop_requested = should_stop or (lambda: False)
    runtime = facebook_runtime or executor.facebook_runtime
    ...
    if not stop_requested() and not executor.runtime_restart_requested():
        due_targets = schedule_planner.list_due_targets(
            ...
            max_count=max(int(options.max_concurrent_scans), 1),
        )
    ...
    if not due_targets and not stop_requested() and not executor.runtime_restart_requested():
        metadata_refresh_count = await _run_resident_maintenance_task(
            resident_metadata_refresh.refresh_requested_target_metadata(
                options=options,
                browser_context=browser_context,
                should_stop=stop_requested,
    ...
    if not due_targets and not stop_requested() and not executor.runtime_restart_requested():
        cover_image_refresh_count = (
            await _run_resident_maintenance_task(
                resident_cover_image_refresh.refresh_pending_target_cover_images(
                    options=options,
                    browser_context=browser_context,
                    should_stop=stop_requested,
                    request_runtime_restart=executor.request_runtime_restart,
...
async def _run_resident_maintenance_task(
    work: Coroutine[Any, Any, int],
    *,
    facebook_runtime: FacebookAutomationRuntime,
) -> int:
    """以獨立 task 執行 maintenance，讓 peer trip 可取消 I/O 而不取消 supervisor。"""
    ...
    try:
        ...
        if facebook_runtime.signal.is_tripped() and not (
