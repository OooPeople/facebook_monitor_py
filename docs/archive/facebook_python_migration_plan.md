# Facebook Group / Post Monitor Python 版重構計劃（封存）

> 文件狀態：  
> 本文件已封存，只保留早期遷移設計與架構背景。  
> 目前實作狀態、下一步、風險與不做事項以 `../TASK_BREAKDOWN.md` 為準。  
> 穩定架構事實以 `../ARCHITECTURE.md` 為準。  
> 新對話或下一位 agent 交接以 `../HANDOFF.md` 為準。  
> 若本文件與目前文件衝突，以目前文件為準。
