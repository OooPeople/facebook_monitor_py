"""Phase 0 無頭 worker probe。

職責：重用 headed setup 建好的專用 profile，確認無頭 worker 能開啟
Facebook group target，並執行最小貼文抽取、關鍵字比對與 seen-item 去重。
"""
...
from phase0_extractors import ExtractRoundStats
from phase0_extractors import collect_items_with_diagnostics
from phase0_extractors import count_matches
from phase0_extractors import make_item_key
from phase0_notifications import NtfyConfig, send_ntfy_notification
...
SEEN_PATH = ROOT / "data" / "runtime" / "phase0_seen_keys.json"
...
def parse_args() -> argparse.Namespace:
    """解析 Phase 0 worker probe 的最小 CLI 參數。"""
    ...
    parser.add_argument(
        "--reset-seen",
        action="store_true",
        help="Clear the local Phase 0 seen-key store before scanning.",
    )
    parser.add_argument(
        "--duration-minutes",
        type=float,
    ...
    parser.add_argument(
        "--ntfy-topic",
        default="",
        help="ntfy topic for Phase 0 test notifications.",
    )
    parser.add_argument(
        "--ntfy-server",
...
def classify_exception(error: Exception) -> str:
    """將 Playwright 或環境例外轉成 Phase 0 失敗分類。"""
...
def run_probe(args: argparse.Namespace) -> ScanResult:
    ...
    if not PROFILE_DIR.exists():
        raise ProbeFailure(
            "profile_missing",
            f"Profile does not exist yet. Run phase0_setup_login.py first: {PROFILE_DIR}",
        )
    ...
    try:
        with acquire_profile_lease(PROFILE_DIR, "phase0 worker probe"):
            with sync_playwright() as p:
                ...
                try:
                    ...
                    items, round_stats = collect_items_with_diagnostics(
                        page=page,
                    ...
                    matched_count = count_matches(items, include_keywords)
...
def dispatch_scan_notification(args: argparse.Namespace, result: ScanResult) -> None:
    ...
    ntfy_result = send_ntfy_notification(
        ...
        title="Facebook Phase 0 scan",
        message=(
            "Phase 0 probe detected items.\n"
...
def send_test_notification(args: argparse.Namespace) -> int:
    ...
    ntfy_result = send_ntfy_notification(
        config=NtfyConfig(server=args.ntfy_server, topic=args.ntfy_topic),
        title="Facebook Phase 0 test",
        message="Phase 0 ntfy test notification.",
    )
    status = "ok" if ntfy_result.ok else "failed"
    log(
        "notification_test "
