...
- Windows version metadata 目前對齊 `0.2.0`；code signing 本輪不做。
...
```text
dist\facebook-monitor-0.2.0-windows-portable.zip
dist\facebook-monitor-0.2.0-windows-portable.zip.sha256
```
...
```powershell
$zip = "dist\facebook-monitor-0.2.0-windows-portable.zip"
...
```powershell
.\scripts\uv.ps1 run python scripts\admin\release_artifact_validation.py --expected-tag v0.2.0
```
...
- `dist\facebook-monitor\facebook-monitor.exe` 存在，`FileVersion=0.2.0.0`、`ProductVersion=0.2.0`。
