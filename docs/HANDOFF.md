...
- Windows updater 穩定邊界看 `docs/ARCHITECTURE.md#windows-updater`；手動 smoke 與打包規則看 `packaging/README.md`。
- 若修改 updater 或 release asset 命名，至少跑 `tests\updates`、settings route 測試、`node --check`，並確認 GitHub tag、zip 檔名與 `.sha256` 內容完全對齊。
- `UPDATER_PLAN.md` 仍是暫時計畫文件；功能封口後應刪除或歸檔，不要讓它變成第二份架構文件。
