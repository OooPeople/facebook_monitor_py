...
- Windows version metadata 目前對齊 `0.3.2`；code signing 本輪不做。
...
```text
dist\facebook-monitor-0.3.2-windows-portable.zip
dist\facebook-monitor-0.3.2-windows-portable.zip.sha256
```
...
若 tag 是 `v0.3.2`，zip 檔名也必須是 `facebook-monitor-0.3.2-windows-portable.zip`。不要把 rc 測試 build 上傳成正式 stable release asset。
...
```text
dist/facebook-monitor-0.3.2-macos-arm64-onedir.zip
dist/facebook-monitor-0.3.2-macos-arm64-onedir.zip.sha256
```
...
```powershell
$zip = "dist\facebook-monitor-0.3.2-windows-portable.zip"
...
```powershell
.\scripts\uv.ps1 run python scripts\admin\release_artifact_validation.py --expected-tag v0.3.2
```
...
- `dist\facebook-monitor\facebook-monitor.exe` 存在，`FileVersion=0.3.2.0`、`ProductVersion=0.3.2`。
