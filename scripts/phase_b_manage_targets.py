"""Phase B interactive target settings manager。

職責：提供暫時的互動式設定入口，讓使用者用選單編輯 target 的關鍵字、
排除字與通知設定，避免日常操作依賴大量 CLI 參數。
"""
...
def run_target_action(app: ApplicationContext, target: TargetDescriptor, action: str) -> None:
    ...
    if action == "1":
        ...
        return
    if action == "2":
        updated_target = app.services.targets.start_target(target.id)
        print(f"已啟動: {updated_target.group_name or updated_target.name}")
        return
    if action == "3":
        updated_target = app.services.targets.stop_target(target.id)
...
def run_manager(db_path: Path) -> int:
    ...
    with SqliteApplicationContext(db_path) as app:
        while True:
            ...
            if not targets:
                print("目前沒有 target。請先執行 phase_b_capture_group_posts.py 新增社團。")
                return 0
