...
- Windows version metadata 目前對齊 `0.3.0`；code signing 本輪不做。
...
- macOS Apple Silicon 目前使用 PyInstaller onedir zip，支援 Web UI 下載、SHA256 驗證、handoff、temp updater 套用與重啟路徑；尚未做 Developer ID signing / notarization。
...
```text
dist\facebook-monitor-0.3.0-windows-portable.zip
dist\facebook-monitor-0.3.0-windows-portable.zip.sha256
```
...
若 tag 是 `v0.3.0`，zip 檔名也必須是 `facebook-monitor-0.3.0-windows-portable.zip`。不要把 rc 測試 build 上傳成正式 stable release asset。
...
```text
dist/facebook-monitor-0.3.0-macos-arm64-onedir.zip
dist/facebook-monitor-0.3.0-macos-arm64-onedir.zip.sha256
```
...
```powershell
$zip = "dist\facebook-monitor-0.3.0-windows-portable.zip"
...
```powershell
.\scripts\uv.ps1 run python scripts\admin\release_artifact_validation.py --expected-tag v0.3.0
```
...
- `dist\facebook-monitor\facebook-monitor.exe` 存在，`FileVersion=0.3.0.0`、`ProductVersion=0.3.0`。
