...
Settings 頁「問題回報與診斷」內的「下載支援診斷包」會建立 redacted zip，內容包含 runtime 診斷、路徑摘要、redacted 最近 log 片段、scheduler / target runtime snapshot、掃描摘要、通知與 dedupe 摘要、DB schema / table counts / invariant 檢查結果。診斷包刻意不包含 SQLite DB、browser profile、cookies、secrets、完整 logs、完整貼文/留言內容或完整 webhook；分享前仍應先解壓檢查內容。
