...
職責：描述 scan commit helper 的結果分類與 worker 後續決策資訊；
本模組不執行 DB、notification 或 scanner side effects。
"""
...
class ScanCommitOutcome:
    """保存 scan commit 結果與 worker 後續決策資訊。"""
    ...
    def committed_visible_scan_state(self) -> bool:
        ...
        return self.scan_run_id > 0 and self.kind in {
