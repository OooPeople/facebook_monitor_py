...
Windows portable EXE 與 macOS Apple Silicon onedir 版本可在設定頁的「程式更新」區塊手動檢查 GitHub stable Release。source mode 只提供檢查資訊，不提供自動套用更新。macOS zip 解壓後請從 `facebook-monitor/Facebook Monitor.app` 啟動；它會顯示在 Dock，並避免 Finder 直接開 Unix executable 時跳出 Terminal。
