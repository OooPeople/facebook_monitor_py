...
def clear_target_seen_baseline_action(db_path: Path, target_id: str) -> TargetActionOutcome:
    """清除 target seen baseline，保留命中紀錄與通知紀錄。"""

    with SqliteApplicationContext(db_path) as app_context:
        result = app_context.repositories.maintenance.clear_target_seen_baseline(target_id)
    if not result.target_found:
        return TargetActionOutcome(ok=False, message="資料清除失敗：target 不存在")
    return TargetActionOutcome(
        ok=True,
        message=f"已清除 seen baseline（{result.seen_items} 筆）",
        feedback="target_seen_cleared",
    )


def clear_target_match_history_action(db_path: Path, target_id: str) -> TargetActionOutcome:
    """清除 target 命中紀錄，保留 seen baseline。"""

    with SqliteApplicationContext(db_path) as app_context:
        result = app_context.repositories.maintenance.clear_target_match_history(target_id)
    if not result.target_found:
        return TargetActionOutcome(ok=False, message="資料清除失敗：target 不存在")
    return TargetActionOutcome(
        ok=True,
        message=f"已清除命中紀錄（{result.match_history} 筆）",
        feedback="target_history_cleared",
    )


def clear_target_notification_data_action(db_path: Path, target_id: str) -> TargetActionOutcome:
    """清除 target 通知事件與 outbox rows。"""

    with SqliteApplicationContext(db_path) as app_context:
        result = app_context.repositories.maintenance.clear_target_notification_data(target_id)
    if not result.target_found:
        return TargetActionOutcome(ok=False, message="資料清除失敗：target 不存在")
    return TargetActionOutcome(
        ok=True,
        message=(
            "已清除通知資料"
            f"（events {result.notification_events} 筆，outbox {result.notification_outbox} 筆）"
        ),
        feedback="target_notifications_cleared",
    )
