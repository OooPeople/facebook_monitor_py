"""Phase B group posts worker tests。"""
...
from facebook_monitor.worker.runner import select_target
...
def test_scan_group_posts_page_records_seen_match_and_scan(tmp_path: Path) -> None:
    ...
    with SqliteApplicationContext(db_path) as app:
        ...
        assert latest_scan.metadata["sort_adjust"]["reason"] == "auto_adjust_sort_disabled"
...
def test_scan_group_posts_page_honors_auto_load_more_config(tmp_path: Path) -> None:
...
def test_scan_group_posts_page_uses_dynamic_window_limit_for_target_count(
    tmp_path: Path,
) -> None:
...
def test_scan_group_posts_page_records_sort_adjust_result(tmp_path: Path) -> None:
...
def test_scan_group_posts_page_sends_ntfy_for_new_match(tmp_path: Path) -> None:
    ...
    with SqliteApplicationContext(db_path) as app:
        ...
        first_summary = scan_group_posts_page(
            page=FakePage(),
            app=app,
            target=target,
            config=config,
        ...
        second_summary = scan_group_posts_page(
            page=FakePage(),
            app=app,
            target=target,
            config=config,
...
def test_scan_group_posts_page_records_failed_ntfy_event(tmp_path: Path) -> None:
...
def test_scan_group_posts_page_records_skipped_ntfy_when_topic_is_empty(
    tmp_path: Path,
) -> None:
...
def test_scan_group_posts_page_records_reserved_notification_channels(
    tmp_path: Path,
) -> None:
...
def test_scan_group_posts_page_supports_userscript_keyword_rules(tmp_path: Path) -> None:
...
def test_scan_group_posts_page_empty_include_matches_all(tmp_path: Path) -> None:
...
def test_scan_group_posts_page_uses_key_aliases_to_prevent_duplicate_notification(
    tmp_path: Path,
) -> None:
...
def test_select_target_by_group_id_when_multiple_targets_exist(tmp_path: Path) -> None:
    ...
    with SqliteApplicationContext(db_path) as app:
        ...
        selected = select_target(app, target_id="", group_id="222")
