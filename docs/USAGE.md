...
在 GitHub Release 頁面，一般使用者只需要下載自己平台的 zip；同名 `.sha256`、`manifest.json` 與 `.sig` 是自動更新與驗證流程使用的 sidecar 檔。macOS zip 內的 `facebook-monitor/README.txt` 也會附首次開啟步驟。

macOS Apple Silicon 首次從 GitHub Release 用瀏覽器下載 zip 後，若開啟 `Facebook Monitor.app` 時顯示已損毀、無法驗證開發者或被 macOS 阻擋，確認 zip 來源是本專案 GitHub Release 後，可執行：
